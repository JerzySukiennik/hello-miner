...
- `mine()` rule (v1.2): **mine what you face; if nothing is in your way, mine what is under you.** A boulder in the drone's facing direction takes priority over the ore under the drone (the ore regrows, the boulder blocks). `can_mine()` returns True when facing a boulder, so `if can_mine(): mine()` clears boulders too. Without this a boulder ringed by stone was permanently unclearable.
- Grid growth retargets in-flight moves so a drone crossing the old edge lands on the true neighbour; if growth drops a boulder or a drone on that target the move is cancelled and returns False.
